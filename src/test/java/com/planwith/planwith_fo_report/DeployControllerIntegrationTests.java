...
class DeployControllerIntegrationTests {
	...
	void deployCheckReturnsMarker() throws Exception {
		mockMvc.perform(get("/api/planwith-fo-report/deploy-check"))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.service").value("planwith-fo-report"))
				.andExpect(jsonPath("$.marker").value("planwith-fo-report-deploy-v1"))
				.andExpect(jsonPath("$.message").value("planwith-fo-report deploy pipeline ok"));
	}

	@Test
	void loginSucceedsWithConfiguredCredentials() throws Exception {
		mockMvc.perform(post("/api/planwith-fo-report/login")
